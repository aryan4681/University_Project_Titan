 ...
 * Heuns method.
 */
public class HeunsSolver {

    /**
     * This method using Heuns solver updates our position every step size.
     */
    public static void updatePositions(SolarSystem system) {
        ...
        for (int i = 1; i < objects.length; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                k2[i][j] = Data.TIME_STEP * (k1[i][j] * (1.0 / 3.0));
                vk2[i][j] = Data.TIME_STEP * (vk1[i][j] * (1.0 / 3.0));

                k3[i][j] = Data.TIME_STEP * (k2[i][j] * (2.0 / 3.0));
                vk3[i][j] = Data.TIME_STEP * (vk2[i][j] * (2.0 / 3.0));

                k[i][j] = k1[i][j] * (0.25) + k3[i][j] * (0.75);
                vk[i][j] = vk1[i][j] * (0.25) + vk3[i][j] * (0.75);
    ...
    /**
     * This method iterates through Heun's iterative approximation method and returns the approximation
    ...
    public static double experimentResult(double stepSize) {
        ...
        for (int i = 0; i < numberOfIterations; i++) {
            ...
            k2 = stepSize * ((Math.pow(tOld + (1.0 / 3.0) * stepSize + yOld + (1.0 / 3.0) * k1, 2.0)) - 1.0);
            k3 = stepSize * ((Math.pow(tOld + (2.0 / 3.0) * stepSize + yOld + (2.0 / 3.0) * k2, 2.0)) - 1.0);

            yCurrent = yOld + (1.0 / 4.0) * (k1 + 3.0 * k3);
