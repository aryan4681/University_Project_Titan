...
public class RK4Solver {
    ...
    /**
     * This method using Runge-Kutta solver updates our position every step size.
     */
    public static void updatePositions(SolarSystem system) {
        ...
        double[][] k3 = new double[objects.length][3];
        double[][] k4 = new double[objects.length][3];

        double[][] vk1 = new double[objects.length][3];
        double[][] vk2 = new double[objects.length][3];
        double[][] vk3 = new double[objects.length][3];
        double[][] vk4 = new double[objects.length][3];
        ...
        for (int i = 1; i < objects.length; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                k2[i][j] = Data.TIME_STEP * (objects[i].getPosition()[j] + vk1[i][j] * 0.5);
                vk2[i][j] = Data.TIME_STEP * (force[i][j] / objects[i].getMass());

                k3[i][j] = Data.TIME_STEP * (objects[i].getPosition()[j] + vk2[i][j] * 0.5);
                vk3[i][j] = Data.TIME_STEP * (force[i][j] / objects[i].getMass());

                k4[i][j] = Data.TIME_STEP * (objects[i].getPosition()[j] + vk3[i][j]);
                vk4[i][j] = Data.TIME_STEP * (force[i][j] / objects[i].getMass());

                k[i][j] = (1.0 / 6.0) * (k1[i][j] + 2 * k2[i][j] + 2 * k3[i][j] + k4[i][j]);
                vk[i][j] = (1.0 / 6.0) * (vk1[i][j] + 2 * vk2[i][j] + 2 * vk3[i][j] + vk4[i][j]);
    ...
    /**
     * This method iterates through Runge-Kutta's iterative approximation method and returns the approximation
    ...
    public static double experimentResult(double stepSize) {
        ...
        for (int i = 0; i < numberOfIterations; i++) {
            ...
            k2 = stepSize * ((Math.pow(tOld + (1.0 / 2.0) * stepSize + yOld + (1.0 / 2.0) * k1, 2.0)) - 1.0);
            k3 = stepSize * ((Math.pow(tOld + (1.0 / 2.0) * stepSize + yOld + (1.0 / 2.0) * k2, 2.0)) - 1.0);
            k4 = stepSize * ((Math.pow(tOld + stepSize + yOld + k3, 2.0)) - 1.0);

            yCurrent = yOld + (1.0 / 6.0) * (k1 + 2.0 * k2 + 2.0 * k3 + k4);
